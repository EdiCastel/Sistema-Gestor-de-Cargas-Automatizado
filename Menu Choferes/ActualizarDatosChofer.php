...
?>
  <head>
    ...
    <title>Actualizar Datos De Chofer</title>
  </head>
  <body>
  <div class="container">
  <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
      ...
      <h1>Actualizar Datos De Chofer</h1><br>
  <?php
    if(isset($_GET['registrar'])){
      ...
      $res = $conexion->query("UPDATE usuario SET id_usuario=id_usuario,nombre='$nombre',apellido1='$apellido1',apellido2='$apellido2',edad='$edad',genero='$genero',direccion='$direccion',codigo_postal='$codigo_postal',tipo_licencia='$tipo_licencia',ocupacion='Chofer',area='Choferes' WHERE  id_usuario='$id';");
    ?>
        <div class="alert alert-success" role="alert">
        <h4 class="alert-heading">Registro completo!</h4>
        <p>Los datos del chofer han sido actualizados en la base de datos</p>
        </div>
        <?php
    }
  ?> 
      ...
      if(isset($_GET['buscar_usuario'])){
        ...
        if (is_null($res)) {
          echo ("El campo nombre está vacío"); 
            ...
            $buscar = $_GET['buscar'];
            $res = $conexion->query("select id_usuario, nombre, apellido1, apellido2, edad, genero, direccion, codigo_postal, tipo_licencia, ocupacion, area from usuario where nombre='$buscar' and ocupacion='Chofer';");
            while($var_fila=$res->fetch_array()){
                ?>  
                <h3>Datos actuales del chofer</h3>
                <table class="table table-dark">
                  ...
                  <div class="py-5 text-center">
                    <h2>Actualizar los datos del chofer</h2>
                  </div>
                    <form class="fregisttro" id="fregistro" method="get">
                    <h4>Porfavor ingrese el id actual del chofer</h4>
                    <div class="form-group">
                        <label for="exampleFormControlInput1">ID Chofer actual</label>
                        <input type="number" class="form-control" id="id" name="id" placeholder="Id" required>
                    </div>
                    <h4>Ingrese los nuevos datos del chofer</h4>
                    <div class="row">
    ...
    ?>  
      ...
      <div class="row">
        <div class="form-group col">
          <a class="w-100 btn btn-lg btn-primary" href="ActualizarChofer.php" role="button">Atras</a>
        </div>
        <div class="form-group col">
          <a class="w-100 btn btn-lg btn-primary" href="menuchoferes.php" role="button">Salir</a>
        </div>
      </div>
      </form>
    </div>
